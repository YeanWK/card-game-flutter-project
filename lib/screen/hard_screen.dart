...
class HardLevle extends StatefulWidget {
  const HardLevle({super.key});

  @override
  State<HardLevle> createState() => _HardLevleState();
}

class _HardLevleState extends State<HardLevle> with TickerProviderStateMixin {
                      ...
                      Container(
                        ...
                        child: InkWell(
                          onTap: () {
                            ...
                            playMusic(1);
                            Navigator.pop(context);
                            ionicbondProvider.randomIonicbondHard();
                            ionicbondProvider.randomAtomHard();
                            setState(() {
                              isWon = false;
                              wrong = 0;
                              selectAtom.clear();
  ...
  void animationSwiftBoy() {
    _timerBoy = Timer.periodic(const Duration(seconds: 1), (Timer timer) {
      ...
      if (wrong == 5) {
        debugPrint("wrong == 5:$wrong");
        setState(() {
          wrong += 1;
          debugPrint("wrong+= 1:$wrong");
          animationEat();
        });
      }
    });
...
//ขยับขึ้นลง
  ...
  void playsound(String sound) async {
    ...
    playerSound.setVolume(settingProvider.volumnSoundFx);
  }
  ...
  void initState() {
    ...
    animationSwiftBoy();
    animationEat();
    _timerWon = Timer(const Duration(milliseconds: 500), () {});
    _centerController =
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<IonicProvider>(builder: (context, valueProvider, child) {
      quizz = valueProvider.quizzHardRandom;
      return Scaffold(
        body: SafeArea(
          child: Column(
            children: [
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: valueProvider.quizzHardRandom
                                          .map((e) => Padding(
                                                padding:
                                    ...
                                    Text(
                                      valueProvider.nameHard,
                                      textAlign: TextAlign.center,
                                      style: TextStyle(
                                          fontSize: 20, color: Colors.black
                                          // color: isWon
                                          //     ? Colors.black
                                          //     : const Color(0xFFEAE9E8),
                                          ),
                                    ),
                                  ],
                                ),
  ...
  Widget buildCard() {
    ...
    if (ionicbondProvider.cardAtomHard.length <= 10) {
      return SizedBox(
        width: 212.5,
        child: GridView.count(
          ...
          children: ionicbondProvider.cardAtomHard.map((atom) {
            return InkWell(
    ...
    if (ionicbondProvider.cardAtomHard.length <= 12) {
      return SizedBox(
        width: 255,
        child: GridView.count(
          padding: const EdgeInsets.only(top: 20),
    ...
    if (ionicbondProvider.cardAtomHard.length <= 14) {
      return SizedBox(
        width: 297.5,
        child: GridView.count(
          padding: const EdgeInsets.only(top: 20),
